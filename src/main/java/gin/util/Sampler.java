...
public abstract class Sampler implements Serializable {
    ...
    private static final String[] OUT_HEADER = {"PatchIndex", "PatchSize", "Patch", "MethodIndex", "TestIndex", "UnitTest", "RepNumber",
            ...
            "AssertionActualValue", "NoOp", "EditsValid", "Reward", "BandSelected"};
    ...
    @Argument(alias = "dsaik", description = "DeepSeek API key for LLM edits")
    protected String deepSeekKey = "";

    @Argument(alias = "dsnm", description = "DeepSeek API model name for LLM edits; e.g. deepseek-v4-pro or deepseek-v4-flash (deepseek-chat / deepseek-reasoner are deprecated aliases)")
    protected String deepSeekName = "deepseek-v4-pro";

    @Argument(alias = "dsurl", description = "DeepSeek base URL; e.g. https://api.deepseek.com")
    protected String deepSeekBaseUrl = "https://api.deepseek.com";

    @Argument(alias = "dsth", description = "Enable DeepSeek thinking mode (supported by deepseek-v4-pro and deepseek-v4-flash). Default false.")
    protected boolean deepSeekThinking = false;

    @Argument(alias = "dsre", description = "DeepSeek reasoning_effort when thinking is enabled: 'high' or 'max'. Default 'high'.")
    protected String deepSeekReasoningEffort = "high";

    @Argument(alias = "mt", description = "model type; OpenAI, DeepSeek, or a name of an ollama model")
    ...
    protected void setUp() {
        ...
        openAIKey = resolveSecretValue(openAIKey, dotEnvValues, "OPENAI_API_KEY", "OpenAI_API_KEY", "OPENAI_KEY");
        deepSeekKey = resolveSecretValue(deepSeekKey, dotEnvValues, "DEEPSEEK_API_KEY", "DeepSeek_API_KEY");
        
        LLMConfig.openAIKey = openAIKey;
        LLMConfig.openAIModelName = openAIName;
        LLMConfig.deepSeekKey = deepSeekKey;
        LLMConfig.deepSeekModelName = deepSeekName;
        LLMConfig.deepSeekBaseUrl = deepSeekBaseUrl;
        LLMConfig.deepSeekThinking = deepSeekThinking;
        LLMConfig.deepSeekReasoningEffort = deepSeekReasoningEffort;
    ...
    private static Map<String, String> loadDotEnvValues() {
        ...
        try (FileReader reader = new FileReader(dotEnvFile)) {
            ...
            Logger.warn("Unable to load .env file from " + dotEnvFile.getAbsolutePath() + ": " + e.getMessage());
            return Collections.emptyMap();
        }

        Map<String, String> values = new HashMap<>();
        for (String name : properties.stringPropertyNames()) {
            values.put(name, stripWrappingQuotes(properties.getProperty(name)));
        }
        return values;
    }

    private static String resolveSecretValue(String cliValue, Map<String, String> dotEnvValues, String... envVarNames) {
        if (isConfiguredSecret(cliValue)) {
        ...
        for (String envVarName : envVarNames) {
            String fileValue = dotEnvValues.get(envVarName);
            if (isConfiguredSecret(fileValue)) {
    ...
    private static boolean isConfiguredSecret(String value) {
        if (value == null) {
            return false;
        }
        String trimmed = value.trim();
        return !trimmed.isEmpty() && !"demo".equalsIgnoreCase(trimmed);
    }
    ...
    protected UnitTestResultSet testPatch(String targetClass, List<UnitTest> tests, Patch patch, Object metadata) {
        ...
        return testPatchWithoutCounting(targetClass, tests, patch, metadata);
    }

    /**
     * Evaluate a patch without consuming a search-budget index. This is used for
     * per-method baseline measurements that are not generated candidates.
     */
    protected UnitTestResultSet testPatchWithoutCounting(
            String targetClass, List<UnitTest> tests, Patch patch, Object metadata) {
    ...
    protected void writeHeader() {
        ...
        try {
            ...
            outputFileWriter.writeNext(outputHeader());
        } catch (IOException e) {
    ...
    protected static String[] appendColumns(String[] baseColumns, String... extraColumns) {
        String[] columns = Arrays.copyOf(baseColumns, baseColumns.length + extraColumns.length);
        System.arraycopy(extraColumns, 0, columns, baseColumns.length, extraColumns.length);
        return columns;
    }
    ...
    protected void writeResults(UnitTestResultSet resultSet) {
        ...
        writeResults(resultSet, patchCount, DEFAULT_ID, 0.0, 0); // default reward of 0
    }

    protected void writeResults(UnitTestResultSet resultSet, Integer methodID) {

        writeResults(resultSet, patchCount, methodID, 0.0, 0); // default reward of 0
    }

    protected void writeResults(UnitTestResultSet testResultSet, int patchCount, Integer methodID) {
        writeResults(testResultSet, patchCount, methodID, 0.0, 0); // default reward of 0
    }

    protected void writeResults(UnitTestResultSet resultSet, Integer methodID, double reward, int bandSelected) {
        writeResults(resultSet, patchCount, methodID, reward, bandSelected);
    }

    protected void writeResults(UnitTestResultSet resultSet, Integer methodID, double reward, int bandSelected, String... extraColumns) {
        writeResults(resultSet, patchCount, methodID, reward, bandSelected, extraColumns);
    }

    protected void writeResults(UnitTestResultSet resultSet, double reward) {
        writeResults(resultSet, patchCount, DEFAULT_ID, reward, 0);
    }

    protected void writeResults(UnitTestResultSet testResultSet, int patchCount, Integer methodID, double reward, int bandSelected) {
        writeResults(testResultSet, patchCount, methodID, reward, bandSelected, new String[0]);
    }

    protected void writeResults(UnitTestResultSet testResultSet, int patchCount, Integer methodID, double reward, int bandSelected, String... extraColumns) {
        int testIdx = 1;
        for (UnitTestResult result : testResultSet.getResults()) {
            writeResult(patchCount, testIdx++, testResultSet.getPatch(), testResultSet.getValidPatch(), testResultSet.getCleanCompile(), result, methodID, testResultSet.getNoOp(), testResultSet.getEditsValid(), reward, bandSelected, extraColumns);
        }
    }

    protected void writeResult(int patchCount, int testNameIdx, Patch patch, boolean patchValid, boolean compiledOK, UnitTestResult testResult, Integer methodID, boolean patchNoOp, List<Boolean> editsValid, double reward, int bandSelected, String... extraColumns) {
        ...
        String rewardStr = Double.toString(reward);
        String bandSelectedStr = Integer.toString(bandSelected);
        String[] entry = {
                patchIndex,
                patchSize,
                patchDetails,
        ...
        if (extraColumns.length > 0) {
            entry = appendColumns(entry, extraColumns);
        }
    ...
    private void writeResult(int patchCount, int testNameIdx, Patch patch, boolean patchValid, boolean compiledOK, UnitTestResult testResult, Integer methodID, boolean patchNoOp, List<Boolean> editsValid) {
        writeResult(patchCount, testNameIdx, patch, patchValid, compiledOK, testResult, methodID, patchNoOp, editsValid, 0.0, 0);
    }
